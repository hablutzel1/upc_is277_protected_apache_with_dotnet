...
qv.util = {
	...
	color: {
		...
		resolveColorVariable: function(color, opacity) {
			var resolvedColor;
			if (qv.util.startsWith(color, "var(") && qv.util.endsWith(color, ")")) {
				var variableName = color.substr(4, color.length - 5);
				resolvedColor = qv.util.trim(getComputedStyle(document.documentElement).getPropertyValue(variableName));
			}
			else
				resolvedColor = color;
			if (!qv.util.startsWith(resolvedColor, "#"))
				resolvedColor = qv.util.color.standardize_color(resolvedColor);
			if (!opacity || opacity == "1")
				return resolvedColor;
			else {
				var bigint = parseInt(resolvedColor.substring(1), 16);
				var r = (bigint >> 16) & 255;
				var g = (bigint >> 8) & 255;
				var b = bigint & 255;
	...
	function mapaEchartsOptions(container, qViewer) {
		...
		if ((qViewer.MapType == QueryViewerMapType.Choropleth && qViewer.Chart.colorAxis.dataClasses.length != 0) ) { //color Scale
			...
			if (qViewer.MapType == QueryViewerMapType.Choropleth) {
				if (legendStyle.BoxStyle.backgroundColor)
					dataRange.backgroundColor = qv.util.color.resolveColorVariable(legendStyle.BoxStyle.backgroundColor, legendStyle.BoxStyle.opacity);
		...
		function getThemeDefinitions(qViewer) {
			...
			function updateBoxStyle(cssStyle, boxStyle) {
				...
				if (cssStyle.opacity)
					boxStyle.opacity = cssStyle.opacity;
		...
		function setLabelStyle(labelObject, style) {
			...
			if (style.BoxStyle.backgroundColor)
				labelObject.backgroundColor = qv.util.color.resolveColorVariable(style.BoxStyle.backgroundColor, style.BoxStyle.opacity);
		...
		function getTooltipObject(qViewer, tooltipStyle) {
			...
			if (tooltipStyle.BoxStyle.backgroundColor)
				tooltipObject.backgroundColor = qv.util.color.resolveColorVariable(tooltipStyle.BoxStyle.backgroundColor, tooltipStyle.BoxStyle.opacity);
		...
		var option = {
			backgroundColor: qv.util.color.resolveColorVariable(themeDefinitions.BackgroundColor, themeDefinitions.Opacity),
		...
		if (qViewer.MapType == QueryViewerMapType.Bubble) {
			...
			if (qViewer.Chart.colorAxis.dataClasses.length != 0) {
				if (themeDefinitions.Legend.BoxStyle.backgroundColor)
					option.legend.backgroundColor = qv.util.color.resolveColorVariable(themeDefinitions.Legend.BoxStyle.backgroundColor, themeDefinitions.Legend.BoxStyle.opacity);
